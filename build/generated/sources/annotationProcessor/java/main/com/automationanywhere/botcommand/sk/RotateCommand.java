...
public final class RotateCommand implements BotCommand {
  ...
  public Optional<Value> execute(GlobalSessionContext globalSessionContext,
    ...
    if(parameters.get("angle") == null || parameters.get("angle").get() == null) {
      throw new BotCommandException(MESSAGES_GENERIC.getString("generic.validation.notEmpty","angle"));
    }
    ...
    if(parameters.get("angle") != null && parameters.get("angle").get() != null && !(parameters.get("angle").get() instanceof Number)) {
      throw new BotCommandException(MESSAGES_GENERIC.getString("generic.UnexpectedTypeReceived","angle", "Number", parameters.get("angle").get().getClass().getSimpleName()));
    }
    try {
      command.action(parameters.get("imagefile") != null ? (String)parameters.get("imagefile").get() : (String)null ,parameters.get("savefile") != null ? (String)parameters.get("savefile").get() : (String)null ,parameters.get("angle") != null ? (Number)parameters.get("angle").get() : (Number)null );Optional<Value> result = Optional.empty();
