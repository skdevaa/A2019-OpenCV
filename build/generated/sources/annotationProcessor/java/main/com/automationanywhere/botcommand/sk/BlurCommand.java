...
public final class BlurCommand implements BotCommand {
  ...
  public Optional<Value> execute(GlobalSessionContext globalSessionContext,
    ...
    if(parameters.get("blursize") == null || parameters.get("blursize").get() == null) {
      throw new BotCommandException(MESSAGES_GENERIC.getString("generic.validation.notEmpty","blursize"));
    }
    ...
    if(parameters.get("blursize") != null && parameters.get("blursize").get() != null && !(parameters.get("blursize").get() instanceof Number)) {
      throw new BotCommandException(MESSAGES_GENERIC.getString("generic.UnexpectedTypeReceived","blursize", "Number", parameters.get("blursize").get().getClass().getSimpleName()));
    }
    try {
      command.action(parameters.get("imagefile") != null ? (String)parameters.get("imagefile").get() : (String)null ,parameters.get("savefile") != null ? (String)parameters.get("savefile").get() : (String)null ,parameters.get("blursize") != null ? (Number)parameters.get("blursize").get() : (Number)null );Optional<Value> result = Optional.empty();
